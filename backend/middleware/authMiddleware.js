...
const protect = asyncHandler(async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {          // Si existe el encabezado de autorización  y comienza con 'Bearer'
        try {
            ...
            // Obtener los datos del usuario del token a través del id_usuario que está en el payload del token
            // para que cualquier endpoint que use protección tenga acceso a esos datos
            req.user = await User.findById(decoded.id).select('-password');             // Busca el usuario en la base de datos por su ID y excluye el campo de contraseña del resultado
